...
async function store(req,res,next){
    ...
    try{
        ...
        if(!policy.can('create','Product')){
            return res.json({
                error : 1,
                message : 'Anda tidak memilika akses untuk membuat produk'
            })
        }
...
async function update(req,res,next){
    ...
    try{
        ...
        if(!policy.can('update','Product')){
            return res.json({
                error : 1,
                message : 'Anda tidak memilika akses untuk mengupdate produk'
            })
        }
        ...
        if(req.file){
            ...
            src.on("end",async ()=>{
                try{
                    ...
                    product = await Product.findOneAndUpdate({_id : req.params.id},payload,{new:false,runValidators:true});
                    return res.json(product);
...
async function destroy(req,res,next){
    try{
        ...
        if(!policy.can('delete','Product')){
            return res.json({
                error : 1,
                message : 'Anda tidak memilika akses untuk menghapus produk'
            })
        }
